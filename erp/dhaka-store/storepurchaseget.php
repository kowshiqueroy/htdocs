...
if (isset($_REQUEST['submitf'])) {
    ...
    $sql = "SELECT * FROM requisitionlist WHERE (status= '5' OR fqty>0)  AND role='$role' ORDER BY id DESC LIMIT 10";
if(isset($_REQUEST['searchi'])){
    ...
    if (strcmp($si,"All" )  ) {
        ...
        $sql = "SELECT * FROM requisitionlist WHERE (status= '5' OR fqty>0) AND role='$role' AND (item like '%$si%') ORDER BY id DESC ";
    }


    else  {

        $sql = "SELECT * FROM requisitionlist WHERE (status= '5' OR fqty>0)  AND role='$role'  ORDER BY id DESC ";
    }
